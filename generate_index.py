...
# Constants
plots_dir = "plots"
output_dir = "."  # Save .md files in the repo root
image_exts = [".png", ".jpg", ".jpeg", ".pdf"]

def format_md_link(name):
    return name.replace(" ", "_").replace("/", "_").lower()

def make_markdown_header(title):
    return f"# {title} Network Simulation Plots\n"
...
def embed_image(path):
    ...
    if ext == ".pdf":
        return f'<embed src="{path}" width="100%" height="600px" type="application/pdf" style="margin-bottom: 20px;" />\n'
    else:
        return f'<img src="{path}" alt="{os.path.basename(path)}" width="700px" style="margin-bottom: 20px;" />\n'
...
with open(os.path.join(output_dir, "index.md"), "w") as f:
    ...
    f.write("Select a detector network to view detailed plots:\n\n")
    for net in networks:
        link = format_md_link(net) + ".md"
        f.write(f"- [{net}]({link})\n")

# Generate one .md file per network
for net in networks:
